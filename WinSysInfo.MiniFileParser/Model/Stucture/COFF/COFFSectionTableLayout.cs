...
namespace WinSysInfo.MiniFileParser.Model
{
    ...
    /// header does not contain a direct pointer to the section table. Instead, the location 
    /// of the section table is determined by calculating the location of the first byte after 
    ...
    /// header. Entries in the section table are numbered starting from one (1). The code and data 
    /// memory section entries are in the order chosen by the linker. In an image file, the VAs for 
    /// sections must be assigned by the linker so that they are in ascending order and adjacent, 
    ...
    public struct COFFSectionTableLayout
    {
        ...
        /// the section is loaded into memory. For object files, this field is the address of the first byte before
        ...
        /// For executable images, this must be a multiple of FileAlignment from the optional header. If this is less
        /// than VirtualSize, the remainder of the section is zero-filled. Because the SizeOfRawData field is rounded
        ...
        /// be a multiple of FileAlignment from the optional header. For object files, the value should be aligned on 
